...
const COUNTRIES = [
  ...
  { code: 'UK', name: 'United Kingdom', dialCode: '+44', length: 10 },
  { code: 'IN', name: 'India', dialCode: '+91', length: 10 },
  { code: 'AU', name: 'Australia', dialCode: '+61', length: 9 },
  { code: 'DE', name: 'Germany', dialCode: '+49', length: 10 },
  ...
  { code: 'AE', name: 'UAE', dialCode: '+971', length: 9 },
  { code: 'SG', name: 'Singapore', dialCode: '+65', length: 8 },
];

function formatPhoneNumber(digits: string, countryCode: string, maxLength: number) {
...
export default function RegisterPage() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', country: 'United States', countryCode: '+1', phone: '', discord: '', github: '', linkedin: '',
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    if (rawDigits.length !== currentCountryObj.length) {
      setFormError(`Phone number must be exactly ${currentCountryObj.length} digits.`);
  ...
  const updateTeamMemberCount = (count: number) => {
    ...
    if (count > newMembers.length) {
      for (let i = newMembers.length; i < count; i++) newMembers.push({ name: '', phone: '', email: '' });
    } else {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm text-[#8e8ea0] flex items-center gap-1.5">
                    Full Name <span className="text-emerald-500 text-xs">*</span>
                  </label>
                  <input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full bg-[#0a0a0a] border border-[#252525] rounded-xl px-4 py-3 text-white text-[15px] placeholder-[#333] focus:outline-none focus:border-emerald-500/60 transition-colors" />
                </div>
                
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm text-[#8e8ea0] flex items-center gap-1.5">
                    Email Address <span className="text-emerald-500 text-xs">*</span>
                  </label>
                  <input type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full bg-[#0a0a0a] border border-[#252525] rounded-xl px-4 py-3 text-white text-[15px] placeholder-[#333] focus:outline-none focus:border-emerald-500/60 transition-colors" />
                </div>

                <div className="flex flex-col gap-1.5 sm:col-span-2">
                  <label className="text-sm text-[#8e8ea0] flex items-center gap-1.5">
                    Phone Number <span className="text-emerald-500 text-xs">*</span>
                  </label>
                  <div className="flex gap-2">
                    <select 
                      ...
                      className="w-[120px] bg-[#0a0a0a] border border-[#252525] rounded-xl px-3 py-3 text-white text-[15px] focus:outline-none focus:border-emerald-500/60 transition-colors appearance-none text-center"
                    >
                    ...
                    <input 
                      ...
                      onChange={e => {
                        ...
                        const currentCountryObj = COUNTRIES.find(c => c.code === selectedCountryCode) || COUNTRIES[0];
                        if (val.length <= currentCountryObj.length) {
                          const formatted = formatPhoneNumber(val, selectedCountryCode, currentCountryObj.length);
                          setFormData({ ...formData, phone: formatted });
                        }
                      }} 
                      className="flex-1 bg-[#0a0a0a] border border-[#252525] rounded-xl px-4 py-3 text-white text-[15px] placeholder-[#333] focus:outline-none focus:border-emerald-500/60 transition-colors" 
                      placeholder={`Enter ${COUNTRIES.find(c => c.code === selectedCountryCode)?.length || 10}-digit phone number`} 
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm text-[#8e8ea0] flex items-center gap-1.5">
                    Discord Handle <span className="text-emerald-500 text-xs">*</span>
                  </label>
                  <input type="text" required value={formData.discord} onChange={e => setFormData({ ...formData, discord: e.target.value })} className="w-full bg-[#0a0a0a] border border-[#252525] rounded-xl px-4 py-3 text-white text-[15px] placeholder-[#333] focus:outline-none focus:border-emerald-500/60 transition-colors" />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-sm text-[#8e8ea0] flex items-center gap-1.5">
                    GitHub Username <span className="text-[#555] text-xs">(optional)</span>
                  </label>
                  <input type="text" value={formData.github} onChange={e => setFormData({ ...formData, github: e.target.value })} className="w-full bg-[#0a0a0a] border border-[#252525] rounded-xl px-4 py-3 text-white text-[15px] placeholder-[#333] focus:outline-none focus:border-emerald-500/60 transition-colors" />
                </div>

                <div className="flex flex-col gap-1.5 sm:col-span-2">
                  <label className="text-sm text-[#8e8ea0] flex items-center gap-1.5">
                    LinkedIn URL <span className="text-[#555] text-xs">(optional)</span>
                  </label>
                  <input type="url" value={formData.linkedin} onChange={e => setFormData({ ...formData, linkedin: e.target.value })} className="w-full bg-[#0a0a0a] border border-[#252525] rounded-xl px-4 py-3 text-white text-[15px] placeholder-[#333] focus:outline-none focus:border-emerald-500/60 transition-colors" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#111111] border border-[#1e1e1e] rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-xs text-[#555] mt-1">Max 3 additional members</p>
                </div>
